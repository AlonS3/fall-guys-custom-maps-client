...
const MapForm = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      code: Yup.string().required("Code Required").matches(/^[0-9]{4}-[0-9]{4}-[0-9]{4}$/, 'Invalid map code format. Please use the format: xxxx-xxxx-xxxx'),
      category: Yup.string().required("Category Required"),
      files: Yup.array().required("Image Required").min(1, 'Please select at least one file').max(3, 'You can select up to 3 files only')
    }),
  ...
  return (
      <VStack as="form" mx="auto" w={{base: "90%", md: 500}} onSubmit={formik.handleSubmit} bg={useColorModeValue('white', 'gray.700')} p={6} my={12} boxShadow={'lg'} rounded={'xl'}>
        <Heading>Add Map</Heading>
        <FormControl isInvalid={formik.errors.title && formik.touched.title} isRequired>
          <FormLabel>Title</FormLabel>
          <Input name='title' placeholder='enter map name' {...formik.getFieldProps("title")}/>
        ...
        {isError && <Text color={"red.500"} mt={2} fontSize={"sm"} lineHeight={"normal"}>{error?.response?.data?.error ?? "Something went wrong, please try again later"}</Text>}
