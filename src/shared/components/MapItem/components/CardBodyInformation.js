...
const CardBodyInformation = ({ map, mapLinkPath, userLinkPath, handleMapClick }) => {
  return (
    <Flex p={2} flexDirection="column" flexGrow="1" overflow="hidden">
      <Heading size="sm" noOfLines={1} flexShrink={0}>
        <Link as={ReactRouterLink} to={mapLinkPath} onClick={handleMapClick} style={{ width: "100%", display: "block" }}>
          {map.title}
        </Link>
      </Heading>
      ...
      <Flex direction="row" justify="space-between" flexShrink={0}>
        <Text textOverflow="ellipsis" overflow="hidden" textAlign="center" color="teal.600" fontSize="sm" textTransform="none" whiteSpace="nowrap">
          <Link as={ReactRouterLink} to={userLinkPath}>
            {map.creator.nickname}
          </Link>
        </Text>
